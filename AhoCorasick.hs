...
findBackEdge root@(ACNode _ _ None) c = case follow root c of
    Just k -> k   -- Can continue from the root using the specified character, target is the back edge
    Nothing -> root   -- Can't continue, no more back edges to follow, root is the new back edge
findBackEdge node c = case follow node c of
    Just k -> k     -- Can continue from the node using the specified character, target is the back edge
